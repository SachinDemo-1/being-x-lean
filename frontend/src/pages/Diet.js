...
// ─── Meal generation ──────────────────────────────────────────────────────────
// All meal-plan math (BMR → TDEE → target macros → fixed-food subtraction →
// dynamic-food allocation → validation/rebalance) now lives in
// `src/utils/dietEngine.js`. Meal quantities and macros are always derived
// from real per-ingredient nutrition — never hardcoded — so a meal's macro
// chip always equals the sum of that meal's items, and the daily totals
// always equal the sum of every meal.
...
export default function Diet() {
  ...
  function handleSurveySubmit(formData) {
    setLoading(true);
    setTimeout(() => {
      const { meals, macros, targetCalories, tdee } = generateMealPlan(formData);
      setPlanResult({ meals, userInfo: formData, macros, targetCalories, tdee });
