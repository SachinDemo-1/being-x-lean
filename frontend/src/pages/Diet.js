...
function generateDietPlan(baseMeals, targetMacros) {
  ...
  // numbers sum to EXACTLY targetMacros — no drift, regardless of rounding.
    const accurateMeals = mealsWithItems.map(meal => ({
      ...
      macros: {
        protein: Math.round(meal.realMacros.protein*1.2),
        carbs: Math.round(meal.realMacros.carbs*1.2),
        fat: Math.round(meal.realMacros.fat*1.2),

        calories: Math.round(
          meal.realMacros.protein * 6 +
